...
struct Access {
  lock_guard<mutex> lg;
  T& ref_to_value;
  Access(T& v, mutex& m) : lg(m), ref_to_value(v) {}
};
...
public:
  ...
  Access<T> GetAccess() { return Access<T>(value, mutex_); }
  Access<const T> GetAccess() const { return Access<const T>(value, mutex_); }
