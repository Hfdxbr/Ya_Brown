...
void RecursiveParseJson(const Node& root, vector<Spending>& result) {
  const auto category_it = root.AsMap().find("category");
  const auto amount_it = root.AsMap().find("amount");
  if(category_it != amount_it) // Possible only if category_it = amount_it = root.AsMap().end()
    result.push_back({category_it->second.AsString(), amount_it->second.AsInt()});
  else
    for(auto& child : root.AsArray())
      RecursiveParseJson(child, result);
}

vector<Spending> LoadFromJson(istream& input) {
  Document doc = Load(input);
  vector<Spending> result;
  RecursiveParseJson(doc.GetRoot(), result);
  return result;
}
