...
void RecursiveParseXml(const Node& root, vector<Spending>& result) {
  if(root.Name() == "spend")
    result.push_back({root.AttributeValue<string>("category"),
                      root.AttributeValue<int>("amount")});
  else
    for(auto& child : root.Children())
      RecursiveParseXml(child, result);
}

vector<Spending> LoadFromXml(istream& input) {
  Document doc = Load(input);
  vector<Spending> result;
  RecursiveParseXml(doc.GetRoot(), result);
  return result;
}
