...
public:
  ...
  BulkLinearUpdater(const BulkMoneyRemover &rem) : rem_(rem) {}
  ...
  void CombineWith(const BulkLinearUpdater &other) {
    ...
    rem_.delta += other.rem_.delta;
  }

  InOutCome Collapse(InOutCome origin, IndexSegment segment) const {
    return {origin.in * tax_.ComputeFactor() + add_.delta * segment.length(),
            origin.out + rem_.delta * segment.length()};
  }
...
private:
  ...
  BulkTaxApplier tax_;
  BulkMoneyAdder add_;
  BulkMoneyRemover rem_;
};

template <typename Data, typename BulkOperation> class SummingSegmentTree {
public:
...
private:
  ...
  TraverseWithQuery(const NodeHolder &node, IndexSegment query_segment,
                    Visitor visitor) {
...
class BudgetManager : public SummingSegmentTree<InOutCome, BulkLinearUpdater> {
public:
...
template <typename ResultType> struct ReadRequest : Request {
...
struct ComputeIncomeRequest : ReadRequest<double> {
  ...
  double Process(const BudgetManager &manager) const override {
    InOutCome result = manager.ComputeSum(MakeDateSegment(date_from, date_to));
...
struct SpendRequest : ModifyRequest {
  ...
  void Process(BudgetManager &manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneyRemover{daily_outcome});
  }
...
struct PayTaxRequest : ModifyRequest {
  ...
  void Process(BudgetManager &manager) const override {
    manager.AddBulkOperation(MakeDateSegment(date_from, date_to),
                             BulkTaxApplier(tax_percentage));
  }

  Date date_from = START_DATE;
  Date date_to = START_DATE;
  int tax_percentage = TAX_PERCENTAGE;
};
...
template <typename Number> Number ReadNumberOnLine(istream &stream) {
  Number number;
  stream >> number;
  string dummy;
