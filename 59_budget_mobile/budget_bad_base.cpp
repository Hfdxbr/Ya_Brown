...
struct InOutCome {
  ...
  InOutCome &operator+=(const InOutCome &other) {
    in += other.in;
    out += other.out;
    return *this;
  }

  InOutCome operator+(const InOutCome &other) const {
    return {in + other.in, out + other.out};
  }

  InOutCome &MultiplyIn(double k) {
    in *= k;
    return *this;
  }

  InOutCome operator*(double k) const { return {in * k, out * k}; }
};

array<InOutCome, VERTEX_COUNT> tree_values, tree_add;
...
void Push(size_t v, size_t l, size_t r) {
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
      ...
      (tree_add[w].MultiplyIn(tree_factor[v])) += tree_add[v];
      (tree_values[w].MultiplyIn(tree_factor[v])) += tree_add[v] * ((r - l) / 2.0);
    }
  }
  tree_factor[v] = 1;
  tree_add[v] = {0, 0};
...
InOutCome ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
...
void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
  ...
  if (ql <= l && r <= qr) {
    tree_add[v].in += value;
    tree_values[v].in += value * (r - l);
    return;
  }
  Add(v * 2, l, (l + r) / 2, ql, qr, value);
  Add(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
  tree_values[v] =
      (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : InOutCome{0, 0}) +
      (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : InOutCome{0, 0});
}

void Remove(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return;
  }
  Push(v, l, r);
  if (ql <= l && r <= qr) {
    tree_add[v].out += value;
    tree_values[v].out += value * (r - l);
    return;
  }
  Remove(v * 2, l, (l + r) / 2, ql, qr, value);
  Remove(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
  tree_values[v] =
      (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : InOutCome{0, 0}) +
      (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : InOutCome{0, 0});
}
...
void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr,
  ...
  if (ql <= l && r <= qr) {
    ...
    tree_add[v].MultiplyIn(factor);
    tree_values[v].MultiplyIn(factor);
    return;
  }
...
int main() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    if (query_type == "ComputeIncome") {
      auto result = ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to);
      cout << result.in - result.out << '\n';
      ...
      double value;
      cin >> value;
      Remove(1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from));
    }
  }
