...
class CoefAccessor {
 ...
 public:
  CoefAccessor(std::vector<T>& vec, size_t ind) : coeffs_(vec), index(ind){};

  const T& operator*() const {
    if (index >= coeffs_.size()) return zero;
    return coeffs_[index];
  }

  T& operator=(const T& value) {
    if (value != zero && index >= coeffs_.size()) coeffs_.resize(index + 1);
    coeffs_[index] = value;
    Shrink();
    return coeffs_[index];
  }
  ...
  bool operator==(const T& value) const {
    if (index >= coeffs_.size()) return value == T();
    return value == coeffs_[index];
  }

  bool operator==(const CoefAccessor& other) const {
    const T& value = other;
...
template <typename T>
std::ostream& operator<<(std::ostream& os, const CoefAccessor<T>& ca) {
  const T& value = *ca;
  os << value;
  return os;
}
...
public:
  ...
  CoefAccessor<T> operator[](size_t degree) {
    return CoefAccessor<T>(coeffs_, degree);
  }
